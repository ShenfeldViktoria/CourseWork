...
#show: project.with(
 ...
 ("Аналіз проблемної галузі та постановка задачі", "01.10.2024 - 10.10.2024"),
    ...
    ("Нормконтроль", "18.11.2024"),
    ("Попередній захист роботи", "25.11.2024"),
    ("Захист роботи", "02.12.2024"),
  ),
  initial_data: [
    Використовуючи програмний продукт Visual Studio Code та об'єктно-орієнтовнy мова програмування Python, для початкового заданого словника значень розробити програмний застосунок з використанням патерну Factory Method, який повинен містити  спроєктовані концептуальну та логічну моделі даних
  ]
)
...
== Середовище розробки
#set align(left)
Програмне середовище Python разом із Visual Studio було обрано завдяки своїй потужності, гнучкості та багатофункціональності, які надають усі необхідні інструменти для ефективної розробки програмного забезпечення. Поєднання цих технологій забезпечує простоту налаштування середовища та зручний робочий процес, що особливо важливо для сучасної розробки.

Python, як одна з найпопулярніших мов програмування, вирізняється простим синтаксисом, який дозволяє писати чистий і зрозумілий код, що значно знижує поріг входу для початківців. Завдяки високій швидкості розробки Python активно використовується як у великих, так і в малих проєктах. Величезна кількість бібліотек, таких як NumPy, Pandas, SciPy, Django та Flask, забезпечує широкий спектр можливостей, від веб-розробки до наукових обчислень і машинного навчання. Це робить Python універсальним інструментом для розробників із різних галузей, що потребують швидкого створення прототипів та їхнього масштабування.

Visual Studio, у свою чергу, надає професійне інтегроване середовище розробки (IDE), яке включає вбудовані інструменти для налагодження, автодоповнення коду, аналізу продуктивності та тестування, що значно спрощує процес розробки. Зокрема, у Visual Studio легко інтегрувати підтримку Python за допомогою розширень, що дозволяє без зайвих зусиль запускати та налагоджувати Python-код. Visual Studio також підтримує Git для контролю версій, що полегшує співпрацю в команді та управління змінами в коді.

Крім того, Python і Visual Studio є кросплатформеними, що дозволяє розробляти програмне забезпечення для різних операційних систем, таких як Windows, macOS і Linux, без необхідності кардинальної зміни коду. Це робить їх зручним інструментом для команд, які працюють у різних операційних середовищах.

Загалом, поєднання Python та Visual Studio є універсальним  середовищем розробки, яке забезпечує високий рівень гнучкості, продуктивності та зручності для розробників. Це вибір, який підходить як для початківців, так і для досвідчених розробників, охоплюючи потреби у швидкому прототипуванні та створенні складних програмних рішень.
#pagebreak()
#set align(center)
...
def create_pizza(pizza_type: PizzaType) -> Pizza:
    factory_dict = {
        ...
        PizzaType.CARBONARA: PizzaCarbonara,
        PizzaType.HUNTERS: PizzaHunters
    }
    return factory_dict[pizza_type]()

if __name__ == '__main__':
    print("Оберіть тип піци:")
    for pizza in PizzaType:
        print(f"{pizza.value}: {pizza.name}")
    
    try:
        choice = int(input("Введіть номер піци: "))
        chosen_pizza_type = PizzaType(choice)
        my_pizza = create_pizza(chosen_pizza_type)
        print(f'Ви обрали: {chosen_pizza_type.name}, ціна: {my_pizza.get_price()} грн')
    except ValueError:
        print("Помилка: введіть коректний номер у вигляді числа.")
    except KeyError:
        print("Помилка: піци з таким номером немає в меню. Оберіть доступний номер.")
